...
var ZodNumber = class _ZodNumber2 extends ZodType {
  constructor() {
    super(...arguments);
    this.min = this.gte;
    this.max = this.lte;
...
var ZodBigInt = class _ZodBigInt2 extends ZodType {
  constructor() {
    super(...arguments);
    this.min = this.gte;
    this.max = this.lte;
...
var ZodDate = class _ZodDate2 extends ZodType {
  _parse(input) {
    if (this._def.coerce) {
...
var ZodArray = class _ZodArray2 extends ZodType {
  _parse(input) {
...
var ZodObject = class _ZodObject2 extends ZodType {
  constructor() {
    super(...arguments);
    this._cached = null;
...
var ZodDiscriminatedUnion = class _ZodDiscriminatedUnion2 extends ZodType {
  _parse(input) {
    const { ctx } = this._processInputParams(input);
...
var ZodTuple = class _ZodTuple2 extends ZodType {
  _parse(input) {
    const { status, ctx } = this._processInputParams(input);
    if (ctx.parsedType !== ZodParsedType.array) {
      addIssueToContext(ctx, {
...
var ZodRecord = class _ZodRecord2 extends ZodType {
  get keySchema() {
...
var ZodSet = class _ZodSet2 extends ZodType {
  _parse(input) {
    const { status, ctx } = this._processInputParams(input);
...
var ZodFunction = class _ZodFunction2 extends ZodType {
  constructor() {
    super(...arguments);
...
var ZodEnum = class _ZodEnum2 extends ZodType {
  _parse(input) {
...
var ZodPipeline = class _ZodPipeline2 extends ZodType {
  _parse(input) {
    const { status, ctx } = this._processInputParams(input);
    if (ctx.common.async) {
...
var ConfigSchema = external_exports.object({
  stateDirectory: external_exports.string().min(1, "State directory is required"),
  stateFile: external_exports.string().min(1, "State file name is required"),
  pollIntervalMs: external_exports.number().int().min(1e3, "Poll interval must be at least 1000ms"),
  enableAutoRetry: external_exports.boolean(),
  maxRetries: external_exports.number().int().min(0, "Max retries must be non-negative"),
  retryDelayMs: external_exports.number().int().min(1e3, "Retry delay must be at least 1000ms"),
  defaultStallTimeoutMs: external_exports.number().int().min(1e3, "Stall timeout must be at least 1000ms"),
  onFailure: external_exports.object({
    type: external_exports.enum(["callback", "log", "none"]),
    callbackUrl: external_exports.string().url().nullable()
  }),
  stepReport: external_exports.object({
    enabled: external_exports.boolean(),
    onComplete: external_exports.boolean(),
    onStart: external_exports.boolean(),
    onFailure: external_exports.boolean(),
    onStalled: external_exports.boolean(),
    notifyFeishu: external_exports.boolean(),
    feishuUserId: external_exports.string()
  }),
  logLevel: external_exports.enum(["debug", "info", "warn", "error"]),
  sessionMonitor: external_exports.object({
    enabledByDefault: external_exports.boolean(),
    triggerPhrases: external_exports.array(external_exports.string()),
    deactivatePhrases: external_exports.array(external_exports.string()),
    silenceThresholdMs: external_exports.number(),
    reminderText: external_exports.string(),
    maxRemindersPerSession: external_exports.number(),
    pollIntervalMs: external_exports.number(),
    feishuApiAppId: external_exports.string(),
    feishuApiAppSecret: external_exports.string()
  })
});
function validateConfig(config) {
  const withDefaults = { ...DEFAULT_CONFIG };
  ...
  if (configObj.onFailure) {
    const onFailure = configObj.onFailure;
    if (onFailure.type && ["callback", "log", "none"].includes(String(onFailure.type))) {
      withDefaults.onFailure.type = String(onFailure.type);
    }
    if (onFailure.callbackUrl !== void 0 && onFailure.callbackUrl !== null) {
  ...
  if (configObj.logLevel && ["debug", "info", "warn", "error"].includes(String(configObj.logLevel))) {
    withDefaults.logLevel = String(configObj.logLevel);
  }
  if (configObj.stepReport) {
    const sr = configObj.stepReport;
  ...
  if (configObj.sessionMonitor) {
    const sm = configObj.sessionMonitor;
...
function createTask(taskName, agentId, steps, maxRetries, stallTimeoutMs) {
...
async function addTask(config, taskName, agentId, steps, stallTimeoutMs) {
...
function findTaskByName(state, taskName) {
...
function areAllStepsCompleted(task) {
  return task.steps.every((s) => s.status === "completed");
}
function hasStepFailed(task) {
  return task.steps.some((s) => s.status === "failed");
}
async function updateStepStatus(config, taskId, stepId, newStatus, error) {
  const state = await loadState(config);
...
async function heartbeat(config, taskId) {
  const state = await loadState(config);
  const task = findTaskById(state, taskId);
  if (!task) {
    return false;
...
async function checkAndMarkStalledTasks(config) {
  const state = await loadState(config);
  ...
  for (const task of state.tasks) {
    ...
    if (elapsed > task.stallDetection.stallTimeoutMs) {
      task.status = "stalled";
      task.lastUpdated = (/* @__PURE__ */ new Date()).toISOString();
...
function getStalledTasks(state) {
...
async function sendStallReport(taskId, taskName) {
...
// src/hooks-handler.js
import { readFile as readFile2, writeFile as writeFile2, mkdir as mkdir2, access as access2, constants as constants2 } from "fs/promises";
...
var NEVER2 = INVALID2;
var ConfigSchema2 = external_exports2.object({
  stateDirectory: external_exports2.string().min(1, "State directory is required"),
  stateFile: external_exports2.string().min(1, "State file name is required"),
  pollIntervalMs: external_exports2.number().int().min(1e3, "Poll interval must be at least 1000ms"),
  enableAutoRetry: external_exports2.boolean(),
  maxRetries: external_exports2.number().int().min(0, "Max retries must be non-negative"),
  retryDelayMs: external_exports2.number().int().min(1e3, "Retry delay must be at least 1000ms"),
  defaultStallTimeoutMs: external_exports2.number().int().min(1e3, "Stall timeout must be at least 1000ms"),
  onFailure: external_exports2.object({
    type: external_exports2.enum(["callback", "log", "none"]),
    callbackUrl: external_exports2.string().url().nullable()
  }),
  stepReport: external_exports2.object({
    enabled: external_exports2.boolean(),
    onComplete: external_exports2.boolean(),
    onStart: external_exports2.boolean(),
    onFailure: external_exports2.boolean(),
    onStalled: external_exports2.boolean(),
    notifyFeishu: external_exports2.boolean(),
    feishuUserId: external_exports2.string()
  }),
  logLevel: external_exports2.enum(["debug", "info", "warn", "error"]),
  sessionMonitor: external_exports2.object({
    enabledByDefault: external_exports2.boolean(),
    triggerPhrases: external_exports2.array(external_exports2.string()),
    deactivatePhrases: external_exports2.array(external_exports2.string()),
    silenceThresholdMs: external_exports2.number(),
    reminderText: external_exports2.string(),
    maxRemindersPerSession: external_exports2.number(),
    pollIntervalMs: external_exports2.number(),
    feishuApiAppId: external_exports2.string(),
    feishuApiAppSecret: external_exports2.string()
  })
});
function getLogLevelPriority2(level) {
  const priorities = {
    debug: 0,
    info: 1,
    warn: 2,
...
function shouldLog2(messageLevel, currentLevel) {
  return getLogLevelPriority2(messageLevel) >= getLogLevelPriority2(currentLevel);
}
var STATE_VERSION2 = "1.0.0";
var fileLocks2 = /* @__PURE__ */ new Map();
async function acquireLock2(filePath) {
  const lockKey = filePath;
  if (!fileLocks2.has(lockKey)) {
    fileLocks2.set(lockKey, { promise: null, count: 0 });
  }
  const lock = fileLocks2.get(lockKey);
  lock.count++;
  if (lock.promise) {
    await lock.promise;
  }
...
async function ensureDirectory2(config) {
  const dirPath = config.stateDirectory;
  try {
    await access2(dirPath, constants2.F_OK);
  } catch {
    await mkdir2(dirPath, { recursive: true });
  }
}
function getStateFilePath2(config) {
  return join2(config.stateDirectory, config.stateFile);
}
function createEmptyState2() {
  return {
    tasks: [],
    version: STATE_VERSION2
  };
}
async function loadState2(config) {
  const release = await acquireLock2(getStateFilePath2(config));
  try {
    await ensureDirectory2(config);
    const filePath = getStateFilePath2(config);
    try {
...
async function saveState2(config, state) {
  const release = await acquireLock2(getStateFilePath2(config));
  try {
    await ensureDirectory2(config);
    const filePath = getStateFilePath2(config);
    state.version = STATE_VERSION2;
    const content = JSON.stringify(state, null, 2);
...
async function checkAndMarkStalledTasks2(config) {
  const state = await loadState2(config);
  const now = (/* @__PURE__ */ new Date()).getTime();
  const stalledIds = [];
  for (const task of state.tasks) {
    if (task.status !== "running" || !task.stallDetection.enabled) {
    ...
    if (elapsed > task.stallDetection.stallTimeoutMs) {
      ...
      task.lastUpdated = (/* @__PURE__ */ new Date()).toISOString();
      stalledIds.push(task.id);
      if (shouldLog2("warn", config.logLevel)) {
        console.warn(`[rloop] Task ${task.id} stalled (no heartbeat for ${elapsed}ms)`);
      }
    }
  }
  if (stalledIds.length > 0) {
    await saveState2(config, state);
  }
  return stalledIds;
}
function getStalledTasks2(state) {
  return state.tasks.filter((t) => t.status === "stalled");
}
var MONITOR_STATE_FILE = "session-monitor.json";
function getMonitorStateFilePath(config) {
  return join2(config.stateDirectory, MONITOR_STATE_FILE);
}
function createEmptyMonitorState() {
  return {};
}
async function loadMonitorState(config) {
  const release = await acquireLock2(getMonitorStateFilePath(config));
  try {
    await ensureDirectory2(config);
...
async function saveMonitorState(config, state) {
  const release = await acquireLock2(getMonitorStateFilePath(config));
  try {
    await ensureDirectory2(config);
    const filePath = getMonitorStateFilePath(config);
...
async function enableSessionMonitor(config, sessionId, silenceThresholdMs, feishuUserId) {
...
async function updateSessionLastMessage(config, sessionId) {
  const state = await loadMonitorState(config);
...
async function getSessionMonitorStatus(config, sessionId) {
  const state = await loadMonitorState(config);
...
async function shouldTriggerReminder(config, sessionId) {
...
var configInstance2 = null;
async function triggerStallCheck() {
  if (!configInstance2) {
    return [];
  }
  try {
    return await checkAndMarkStalledTasks2(configInstance2);
  } catch {
    return [];
  }
}
function formatStalledTaskWarning(stalledTasks) {
  if (stalledTasks.length === 0) return "";
  let warning = "\n\n\u26A0\uFE0F RLOOP STALLED TASK WARNING:\n";
  warning += "Detected stalled task(s) that need continuation:\n\n";
  for (const task of stalledTasks) {
    warning += `\u{1F4CB} Task: "${task.taskName}" (ID: ${task.id})
`;
...
function extractFeishuUserIdFromMessages(messages) {
  if (!messages || !Array.isArray(messages)) return void 0;
  for (let i = messages.length - 1; i >= 0; i--) {
    const msg = messages[i];
    if (!msg) continue;
    ...
    const channelContext = msg.channel_context;
...
async function handleSessionMonitoring(config, sessionId, messages, feishuUserId) {
...
async function handleBeforePromptBuild(api, config, event) {
  if (shouldLog2("debug", config.logLevel)) {
  ...
  try {
    ...
    const state = await loadState2(config);
    const stalledTasks = getStalledTasks2(state);
    ...
    if (shouldLog2("error", config.logLevel)) {
      console.error("[rloop] Error in before_prompt_build:", error);
    }
  }
}
async function handleBeforeAgentStart(api, config, event) {
  if (shouldLog2("debug", config.logLevel)) {
  ...
  try {
    ...
    const state = await loadState2(config);
    const stalledTasks = getStalledTasks2(state);
    if (stalledTasks.length > 0 && shouldLog2("info", config.logLevel)) {
...
async function handleAfterToolCall(api, config, event) {
...
function getFeishuClient() {
  try {
    const { LarkClient } = __require("@larksuite/openclaw-lark");
    return LarkClient.get("default") ?? null;
  } catch {
    return null;
  }
}
function sendFeishuMarkdown(to, text) {
...
async function sendStepReport(config, api, event) {
  if (!config.stepReport.enabled) return;
  const shouldSend = event.type === "completed" && config.stepReport.onComplete || event.type === "started" && config.stepReport.onStart || event.type === "failed" && config.stepReport.onFailure;
  ...
  const label = emoji[event.type] ?? "\u{1F4CB}";
  const nextStep = event.type === "completed" ? event.stepId + 1 : event.stepId;
  const nextDesc = event.type === "completed" ? "" : ` \xB7 \u4E0B\u4E00\u6B65\uFF1A${event.stepDesc}`;
...
function registerTools(api, config) {
  ...
  api.registerTool({
    ...
    execute: async (_toolCallId, params) => {
      ...
      if (!taskId || stepId === void 0 || !status) {
  ...
  api.registerTool({
    ...
    execute: async (_toolCallId, params) => {
      const status = params.status;
      const includeStalled = params.includeStalled ?? true;
      try {
        const state = await loadState(config);
...
function resolveStateDirectory(api, configuredPath) {
  ...
  if (workspace) {
    const statePath2 = join3(workspace, "state");
    if (!existsSync(statePath2)) {
      mkdirSync(statePath2, { recursive: true });
    }
    return statePath2;
  }
  const pluginDir = dirname2(__require.resolve("./index.js"));
  const statePath = join3(pluginDir, "state");
